...
function initThreeJS() {
    ...
    // --- THAY ĐỔI: TẠO HỘP QUÀ VỚI TEXTURE ---
    const textureLoader = new THREE.TextureLoader();
    const wrapTexture = textureLoader.load('wrapping_paper.jpg'); // Tải ảnh giấy gói
    const ribbonTexture = textureLoader.load('ribbon.jpg');     // Tải ảnh nơ

    // Lặp lại texture giấy gói nếu cần
    wrapTexture.wrapS = THREE.RepeatWrapping;
    wrapTexture.wrapT = THREE.RepeatWrapping;
    wrapTexture.repeat.set(2, 2); // Lặp lại 2x2 lần (tùy chỉnh)

    // Tạo vật liệu cho từng mặt của hộp
    const boxMaterials = [
    ...
    giftBox = new THREE.Mesh(boxGeometry, boxMaterials); // Áp dụng mảng vật liệu
    giftBox.name = "giftBox";
    scene.add(giftBox);

    // Thêm ánh sáng để thấy vật liệu Phong
    ...
    disc1 = createDisc(22, 10, 5000, 0, 0.97, textureLoader, 0x8A2BE2);
    disc2 = createDisc(9, 5, 1500, 0.2, 1.1, textureLoader, 0x8A2BE2);
    scene.add(disc1);
    scene.add(disc2);
...
function animateThreeJS() {
    ...
    if (pointPlanet) pointPlanet.rotation.y += 0.002;
    // Thêm xoay nhẹ cho hộp quà
    if (giftBox) giftBox.rotation.y += 0.005;

    if (disc1) disc1.rotation.y -= 0.01;
    if (disc2) disc2.rotation.y -= 0.02;
    if (starField) starField.rotation.y += 0.0001;
    if (ambientLights) {
        ambientLights.rotation.y += 0.0002;
        ambientLights.rotation.z += 0.0001;
    }
    controls.update();
...
// --- HÀM XỬ LÝ CLICK TRÊN CANVAS 3D ---
// ... (Giữ nguyên)
